...
@warning_ignore("unused_signal") # reserved for future use; see Y4.1 validation doc
...
func refresh_yandex_sdk_ready() -> bool:
	...
	var result = JavaScriptBridge.eval("""
		(function() {
			try {
				return (window.ysdk && (window.ysdkReady === undefined || window.ysdkReady === true)) ? 1 : 0;
			} catch(e) {
				return 0;
			}
		})();
	""")
	is_yandex_available = _variant_to_bool(result, false)
...
# Safely normalizes JavaScriptBridge.eval() results — Web exports may return
# int (1/0) instead of bool for JS boolean values, and Godot 4.5 errors on
# int == bool comparisons (see Y4.3 hotfix).
...
func _on_js_rewarded_ad_open(_args: Array) -> void:
...
func _on_js_rewarded_ad_rewarded(_args: Array) -> void:
...
func _on_js_fullscreen_ad_open(_args: Array) -> void:
...
func _setup_platform_event_callbacks(attempt: int = 0) -> void:
	...
	var has_on = JavaScriptBridge.eval("""
		(function() {
			try { return (window.ysdk && typeof window.ysdk.on === 'function') ? 1 : 0; } catch(e) { return 0; }
		})();
	""")
	if not _variant_to_bool(has_on, false):
