...
- Russian is the safe default/fallback for the Yandex release. English
  localization remains supported and must not be removed unless requested.
  English literals are limited to internal ids, product/reward ids, paths,
  comments, and debug/dev-only logs; an unavoidable visible fallback must be
  Russian.
...
- The desktop-only Web backdrop source is `assets/images/web/desktop_backdrop.png`. It is copied next to
  the Web export by `python tools/package_yandex_web.py`; use it only in `web/yandex_shell.html`, never
  in Godot scenes or UI asset catalogs. The centered Web canvas remains 720x1280 (9:16); Android and
  portrait mobile layout must not change.
...
- **All ad and purchase rewards must go through `ClickerState` grant helpers.**
  Helpers may mutate game state only; they must not call UI, `Platform`, or
  `SaveManager`. Rewarded-ad helpers may be called only from the rewarded
  callback. Paid rewards require a valid, unprocessed token, and must be
  saved/flushed before `Platform.consume_purchase()`.
...
- **Web/Yandex language must be authoritative from the Yandex SDK
  (Y4.2).** `ClickerScreen._apply_startup_language()` reads
  `Platform.get_platform_language()` (→ `ysdk.environment.i18n.lang`) on
  Web/Yandex unconditionally — `state.language_manually_selected` (including
  from old saves) must never block it there. Do not let a saved/manual
  language override SDK language on Web/Yandex.
- **Do not expose the manual language toggle on Web/Yandex** unless Yandex
  moderation requirements change. `SettingsWindow` only creates the language
  row when `Platform.get_platform_key() != "yandex"`. Android/Local keep
  manual language selection unchanged. Language fixes must not touch
  payments, ads, saves, or gameplay balance.
...
- **Never compare a raw `JavaScriptBridge.eval()` result directly to
  `true`/`false` (Y4.3).** Web exports can return an `int` (`1`/`0`) instead
  of a JS `bool`, and Godot 4.5 throws `Invalid operands 'int' and 'bool'` on
  `int == bool`. Always normalize through `YandexBridge._variant_to_bool(value,
  default_value)` (handles `bool`/`int`/`float`/`String`/unexpected types)
  instead of `result == true` / `result != true`.
- **SDK-ready checks must never throw at runtime (Y4.3).**
  `YandexBridge.refresh_yandex_sdk_ready()` and `_is_ysdk_ready()` are called
  from every SDK-dependent path (language, ads, catalog, cloud save) — a
  thrown error there cascades into all of them. Keep the JS side wrapped in
  `try/catch` returning a safe value, and the GDScript side going through
  `_variant_to_bool()`.
- **Web/Yandex release must never run debug visual test mode (Y4.3).**
  `ClickerState.is_debug_visual_test_mode_enabled()` /
  `set_debug_visual_test_mode_enabled()` force it off whenever
  `Platform.get_platform_key() == "yandex"` and `BuildConfig.IS_DEBUG_BUILD`
  is `false`, on top of the existing F12-toggle gate in
  `ClickerScreen._input()`. Do not remove either gate independently.
- **Do not mix the JavaScriptBridge bool/int interop hotfix (Y4.3) with
  gameplay/payment reward changes.** Y4.3 only touches SDK-readiness
  plumbing; purchase/consume/recovery logic must stay byte-for-byte
  unchanged in the same patch.
...
- `ZoneConfig.ZONE_COUNT = 10`; each zone has 5 stages, so the visual/content cycle repeats every 50 stages while real stage numbers remain infinite.
- Each active zone owns one same-numbered background folder and one same-numbered `boss_01` folder.
- Keep the existing non-boss enemy pools unchanged until the separate T2 enemy-pool simplification.
