...
const FULLSCREEN_AD_WARNING_SECONDS: float = 5.0
const REWARDED_AD_CLOSE_REWARD_GRACE_SECONDS: float = 0.5
...
var _fullscreen_ad_warning_label: Label = null
...
func _process(delta: float) -> void:
	...
	if _fullscreen_ad_cooldown_left > 0.0:
		...
		_update_fullscreen_ad_warning()
	elif _is_initialized:
		_hide_fullscreen_ad_warning()
...
func _on_payment_purchase_success(local_product_id: String, purchase_token: String) -> void:
	...
	if local_product_id != _pending_payment_product_id:
		...
		_clear_payment_request_state()
		return
	if GemPurchaseConfigClass.get_by_id(local_product_id).is_empty():
		push_warning("Platform: success for unknown product '%s', ignoring" % local_product_id)
		AudioManager.play_purchase_error()
		_handle_status_text(LocalizationManager.tr_key("shop.gem_purchase.error"))
		_clear_payment_request_state()
		return
	...
	if not result.get("upgraded", false):
		push_warning("Platform: paid purchase grant failed for '%s'" % local_product_id)
		AudioManager.play_purchase_error()
		_handle_status_text(result.get("status_text", ""))
		_clear_payment_request_state()
		return
...
func _on_unprocessed_purchase_found(product_id: String, purchase_token: String) -> void:
	if purchase_token == "":
		push_warning("Platform: unprocessed purchase for '%s' without purchase id, ignoring" % product_id)
		return
	if state.is_purchase_processed(purchase_token):
		return
	...
	if local_product_id == "":
		push_warning("Platform: unprocessed purchase product '%s' has no local id, ignoring" % product_id)
		return
	var result: Dictionary = state.grant_paid_gem_purchase(local_product_id)
	if not result.get("upgraded", false):
		push_warning("Platform: unprocessed purchase grant failed for '%s'" % local_product_id)
		return
	_handle_status_text(result.get("status_text", ""))
	AudioManager.play_purchase_success()
	_update_ui()
...
func _create_fullscreen_ad_warning() -> void:
	_fullscreen_ad_warning_label = Label.new()
	_fullscreen_ad_warning_label.name = "FullscreenAdWarning"
	_fullscreen_ad_warning_label.anchor_left = 0.5
	_fullscreen_ad_warning_label.anchor_right = 0.5
	_fullscreen_ad_warning_label.anchor_top = 0.5
	_fullscreen_ad_warning_label.anchor_bottom = 0.5
	_fullscreen_ad_warning_label.offset_left = -300
	_fullscreen_ad_warning_label.offset_right = 300
	_fullscreen_ad_warning_label.offset_top = -45
	_fullscreen_ad_warning_label.offset_bottom = 45
	_fullscreen_ad_warning_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	_fullscreen_ad_warning_label.vertical_alignment = VERTICAL_ALIGNMENT_CENTER
	_fullscreen_ad_warning_label.mouse_filter = Control.MOUSE_FILTER_IGNORE
	_fullscreen_ad_warning_label.add_theme_font_size_override("font_size", 32)
	_fullscreen_ad_warning_label.add_theme_color_override("font_color", Color(1, 1, 1, 1))
	_fullscreen_ad_warning_label.add_theme_constant_override("outline_size", 5)
	_fullscreen_ad_warning_label.add_theme_color_override("font_outline_color", Color(0, 0, 0, 0.9))
	_fullscreen_ad_warning_label.visible = false
	add_child(_fullscreen_ad_warning_label)


func _update_fullscreen_ad_warning() -> void:
	if not is_instance_valid(_fullscreen_ad_warning_label):
		return
	if _fullscreen_ad_cooldown_left > FULLSCREEN_AD_WARNING_SECONDS or _is_purchase_ui_active():
		_hide_fullscreen_ad_warning()
		return
	var seconds_left: int = maxi(1, ceili(_fullscreen_ad_cooldown_left))
	_fullscreen_ad_warning_label.text = LocalizationManager.format_key(
		"fullscreen_ad.warning_countdown", {"seconds": str(seconds_left)}
	)
	_fullscreen_ad_warning_label.visible = true


func _hide_fullscreen_ad_warning() -> void:
	if is_instance_valid(_fullscreen_ad_warning_label):
		_fullscreen_ad_warning_label.visible = false


func _is_purchase_ui_active() -> bool:
	return (
		_pending_payment_product_id != ""
		or _pending_shop_product_id != ""
		or gem_purchase_dialog.visible
		or shop_purchase_confirm_dialog.visible
	)
...
func _apply_startup_language() -> void:
	if Platform.get_platform_key() == "yandex":
		# Y4.2: Yandex SDK language is authoritative on Web — a saved manual
		# selection (including from old saves) must never block it.
		if state.language_manually_selected:
			state.language_manually_selected = false
			_save_game_now()
...
func _apply_startup_language_when_platform_ready(attempt: int = 0) -> void:
	...
	if not is_yandex and state.language_manually_selected:
		return
	...
	if platform_lang != "":
		...
		var needs_save: bool = resolved != state.language or state.language_manually_selected
...
func _on_language_manually_changed(_language_code: String) -> void:
	if Platform.get_platform_key() == "yandex":
		# Safety net: manual language toggle should be hidden on Web/Yandex,
		# but if it's ever reachable, don't let it override SDK language.
		state.language_manually_selected = false
		return
...
func _on_rewarded_ad_rewarded() -> void:
	...
	match _rewarded_ad_request_context:
		"bonus_banner":
			...
			if not banner_result.get("upgraded", false):
				_handle_status_text(banner_result.get("status_text", ""))
				return
			_handle_status_text(banner_result.get("status_text", ""))
			_show_runtime_status_toast(banner_result.get("status_text", ""))
			AudioManager.play_reward_received()
			state.refresh_derived_stats()
			_update_ui()
			_save_game_and_flush_cloud_now()
		...
		"shop_gems":
			...
			if not shop_result.get("upgraded", false):
				_handle_status_text(shop_result.get("status_text", ""))
				return
			_handle_status_text(shop_result.get("status_text", ""))
			_show_runtime_status_toast(shop_result.get("status_text", ""))
			AudioManager.play_reward_received()
			state.refresh_derived_stats()
			_update_ui()
			_save_game_and_flush_cloud_now()
		"offline_gold_x3":
			var offline_result: Dictionary = state.claim_pending_offline_gold(BalanceConfig.OFFLINE_GOLD_AD_MULTIPLIER)
			if not (offline_result.get("reward_gold", null) is BigNumber):
				return
			offline_reward_dialog.hide_dialog()
			AudioManager.play_reward_received()
			_update_ui()
...
func _on_rewarded_ad_closed(was_shown: bool) -> void:
	...
	_finalize_rewarded_ad_close_after_grace(_rewarded_ad_request_context, was_shown)


func _finalize_rewarded_ad_close_after_grace(closed_context: String, was_shown: bool) -> void:
	await get_tree().create_timer(REWARDED_AD_CLOSE_REWARD_GRACE_SECONDS).timeout
	if _rewarded_ad_request_context != closed_context:
		return
