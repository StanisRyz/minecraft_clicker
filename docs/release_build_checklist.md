...
## Desktop backdrop and Yandex ZIP

Before creating a Yandex Games release ZIP, ensure the replaceable outer-Web
backdrop exists at `assets/images/web/desktop_backdrop.png`, then run:

```sh
python tools/package_yandex_web.py
```

The helper copies the image beside `builds/web/index.html` as
`desktop_backdrop.png`, verifies that the generated shell references it, and
creates the flat `builds/web_release.zip` with `index.html` at ZIP root. The
image is used only by `web/yandex_shell.html` on wide Web viewports; never add
it to Godot scenes or UI catalogs. On desktop, verify that the centered 9:16
game has the backdrop at the sides without black fields. On portrait mobile,
verify that no backdrop is visible and the existing canvas layout is unchanged.
