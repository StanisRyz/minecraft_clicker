...
**Update (Y4.2):** Yandex moderation flagged that automatic language
detection wasn't actually enforced — a saved `language_manually_selected =
true` (including from an old save, or a tester who had once used the
manual toggle) permanently blocked SDK language on Web. SDK language is now
authoritative on Web/Yandex:
- `ClickerScreen._apply_startup_language()` ignores/clears
  `state.language_manually_selected` on Web/Yandex before delegating to
  `_apply_startup_language_when_platform_ready()` — the manual flag can no
  longer block SDK language, old saves included.
- `_apply_startup_language_when_platform_ready()` resets
  `state.language_manually_selected = false` whenever it resolves a
  platform language on Web/Yandex, so later starts keep querying the SDK
  rather than trusting the saved language as authority.
- `SettingsWindow` no longer creates the manual language toggle row on
  Web/Yandex (`Platform.get_platform_key() != "yandex"` gate). Android/Local
  keep the toggle and manual-selection behavior unchanged.
- `ClickerScreen._on_language_manually_changed()` ignores the event on
  Web/Yandex as a safety net in case the toggle is ever reachable.
See `docs/validation/yandex_sdk_language_authority.md`.
