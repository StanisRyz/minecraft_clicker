**Update (Y4.2):** SDK language is now authoritative on Web/Yandex — a saved
`language_manually_selected = true` (including from an old save) no longer
blocks it. The retry loop below is unchanged/preserved. See
`docs/validation/yandex_sdk_language_authority.md`.

**Update (Y4.3):** fixed a `JavaScriptBridge.eval()` bool/int interop bug in
`refresh_yandex_sdk_ready()` that threw `Invalid operands 'int' and 'bool'`
and broke SDK readiness entirely (language, `LoadingAPI.ready()`, ads,
catalog, cloud saves). The retry/timeout logic documented below is
unchanged/preserved. See
