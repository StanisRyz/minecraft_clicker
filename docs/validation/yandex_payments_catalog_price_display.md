...
`refresh_yandex_sdk_ready()` that made `_is_ysdk_ready()` throw/misreport,
which prevented `load_payment_catalog()` from ever reaching
`ysdk.getPayments()`/`payments.getCatalog()` and left the dialog on endless
"Loading price...". Catalog logic itself is unchanged. See
