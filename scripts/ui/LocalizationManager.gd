...
const SUPPORTED_LANGUAGES: Array[String] = ["ru", "en"]
...
func tr_key(key: String) -> String:
	...
	# Russian is the release-safe fallback. English is used only when explicitly selected.
	if _current_language != DEFAULT_LANGUAGE and _translations.has(DEFAULT_LANGUAGE):
		var fallback_map: Dictionary = _translations[DEFAULT_LANGUAGE]
		if fallback_map.has(key):
			var fallback_text: String = fallback_map[key]
			if fallback_text != "":
