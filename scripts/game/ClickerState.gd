...
const ShopConfigData = preload("res://scripts/game/config/ShopConfig.gd")
...
# Legacy save field name retained for Save System v1 compatibility; its active multiplier is x4 from BalanceConfig.
...
func buy_prestige_talents(talent_index: int, mode: String) -> Dictionary:
	if talent_index < 0 or talent_index >= prestige_talent_levels.size():
		return _make_purchase_result("Неверный талант престижа")
	...
	if bought <= 0 or total_cost <= 0 or prestige_points_available < total_cost:
		return _make_purchase_result("Недостаточно очков престижа")

	prestige_points_available -= total_cost
	prestige_talent_levels[talent_index] += bought
	_update_character_state()
	return _make_purchase_result("Талант престижа улучшен x%d!" % bought, false, true)
...
func apply_reset_progress_preserved_snapshot(snapshot: Dictionary) -> void:
	...
	var snap_lang: String = str(snapshot.get("language", "ru"))
	language = snap_lang if snap_lang in ["en", "ru"] else "ru"
...
func perform_prestige() -> Dictionary:
	...
	if reward <= 0:
		return _make_purchase_result(
			"Престиж требует этап %d или уровень героя %d" % [
	...
	return {
		...
		"status_text": "Престиж завершён! +%d очков престижа" % reward,
		"zone_changed": false,
		"zone_name": "",
	}
...
func attack_with_damage(damage) -> Dictionary:
	...
	if not damage_bn.is_positive():
		return _make_attack_result(false, false, BigNumber.zero(), BigNumber.zero(), target_hp.clone(), target_hp.clone(), "Нажмите на поле для атаки!")
	...
	if target_hp.is_positive():
		return _make_attack_result(false, false, BigNumber.zero(), damage_dealt, target_hp_before, target_hp.clone(), "Нажмите на поле для атаки!")
...
func resolve_defeated_target() -> Dictionary:
	...
	var status_text: String = LocalizationManager.format_key("status.enemy_defeated_gold", {"gold": gold_str})
	var zone_changed: bool = false
	var new_zone_name: String = ""
	...
	if is_level_cleared(current_level):
		if auto_stage_advance_enabled:
			...
			if zone_changed:
				status_text = LocalizationManager.format_key("status.new_zone", {"zone": zone_name})
			elif defeated_boss:
				status_text = LocalizationManager.format_key("status.boss_defeated_level", {"gold": gold_str, "level": str(current_level)})
			else:
				status_text = LocalizationManager.format_key("status.level_up", {"level": str(current_level)})
		else:
			enemies_defeated_on_level = enemies_required_per_level
			reset_target()
			save_current_level_progress()
			if defeated_boss:
				status_text = LocalizationManager.format_key("status.boss_defeated_farming", {"gold": gold_str, "level": str(current_level)})
			else:
				status_text = LocalizationManager.format_key("status.farming", {"gold": gold_str, "level": str(current_level)})
	elif did_level_up:
		...
		if auto_stage_advance_enabled:
			...
			save_current_level_progress()
			if defeated_boss:
				status_text = LocalizationManager.format_key("status.boss_defeated_stage_unlocked", {"gold": gold_str, "level": str(next_level)})
			else:
				status_text = LocalizationManager.format_key("status.stage_unlocked", {"gold": gold_str, "level": str(next_level)})
	else:
		reset_target()
		save_current_level_progress()
...
func buy_character_level_upgrades(mode: String) -> Dictionary:
	...
	if bought <= 0 or total_cost.is_zero() or gold.compare_to(total_cost) < 0:
		return _make_purchase_result("Недостаточно золота", true)

	gold = gold.subtract(total_cost)
	character_level += bought
	recalculate_character_level_cost()
	_update_character_state()
	return _make_purchase_result("Уровень героя повышен x%d!" % bought, false, true)
...
func grant_rewarded_ad_bonus(reward_id: String) -> Dictionary:
	if not REWARDED_AD_REWARD_IDS.has(reward_id):
		return {
			"reward_id": "",
	...
	return _make_purchase_result(LocalizationManager.tr_key("rewarded_ad.status.not_available"))
...
func grant_shop_rewarded_gems() -> Dictionary:
	var product: Dictionary = ShopConfigData.get_by_id("rewarded_gems_ad")
	var amount: int = int(product.get("reward_gems", BalanceConfig.SHOP_REWARDED_GEMS_AD_REWARD))
...
func buy_partners(partner_index: int, mode: String) -> Dictionary:
	if partner_index < 0 or partner_index >= partner_counts.size():
		return _make_purchase_result("Неверный партнёр")
...
func buy_buildings(building_index: int, mode: String) -> Dictionary:
	if building_index < 0 or building_index >= building_counts.size():
		return _make_purchase_result("Неверное здание")
...
func buy_hero_skill(skill_id: String) -> Dictionary:
	var skill: Dictionary = get_hero_skill(skill_id)
	if skill.is_empty():
		return _make_purchase_result("Неверный навык героя")
	if is_hero_skill_purchased(skill_id):
		return _make_purchase_result("Навык героя уже куплен")
	if not is_hero_skill_unlocked(skill_id):
		return _make_purchase_result("Требуется уровень героя %d" % int(skill.get("unlock_character_level", 0)))
	var cost: BigNumber = get_hero_skill_cost(skill_id)
	...
	_update_character_state()
	return _make_purchase_result("%s куплено!" % Presentation.get_skill_name(skill), false, true)
...
func buy_ability_skill(skill_id: String) -> Dictionary:
	var skill: Dictionary = get_ability_skill(skill_id)
	if skill.is_empty():
		return _make_purchase_result("Неверный навык способности")
	if is_ability_skill_purchased(skill_id):
		return _make_purchase_result("Навык способности уже куплен")
	...
	if not is_debug_purchase_override_enabled() and not is_ability_purchased(ability_id):
		return _make_purchase_result("Сначала купите %s" % _get_ability_display_name(ability_id))
...
func buy_partner_skill(skill_id: String) -> Dictionary:
	var skill: Dictionary = get_partner_skill(skill_id)
	if skill.is_empty():
		return _make_purchase_result("Неверный навык партнёра")

	if is_partner_skill_purchased(skill_id):
		return _make_purchase_result("Навык партнёра уже куплен")
...
func buy_ability_unlock(ability_id: String) -> Dictionary:
	if is_ability_purchased(ability_id):
		return _make_purchase_result("Уже куплено")
	if not is_ability_unlocked(ability_id):
		return _make_purchase_result("Требуется уровень героя %d" % get_ability_unlock_level(ability_id))
	var cost: BigNumber = get_ability_unlock_cost(ability_id)
	if not cost.is_positive() or gold.compare_to(cost) < 0:
		return _make_purchase_result("Недостаточно золота", true)
	gold = gold.subtract(cost)
	match ability_id:
		...
		_:
			return _make_purchase_result("Неверная способность")
	return _make_purchase_result("%s куплено!" % _get_ability_display_name(ability_id), false, true)


func _get_ability_display_name(ability_id: String) -> String:
	match ability_id:
		"autoclick": return "Автоклик"
		"gold_bonus": return "Бонус золота"
		"focus_burst": return "Фокус"
		"rally": return "Сбор"
	return "Способность"
...
func buy_or_upgrade_ability(ability_id: String) -> Dictionary:
	...
	return _make_purchase_result("Уже максимальный ранг")
...
func enable_auto_stage_advance_and_jump_if_needed() -> Dictionary:
	...
	if is_level_cleared(current_level) and current_level < max_unlocked_level:
		...
		return {
			...
			"status_text": "Автопереход включён. Переход на уровень %d." % current_level,
		}
	return {
		"moved_to_latest": false,
		"advanced_to_next_level": false,
		"current_level": current_level,
		"status_text": "Автопереход включён.",
	}
...
func fail_boss_level() -> Dictionary:
	if is_boss_level and boss_retry_tokens > 0:
		...
		return {
			...
			"status_text": "Повтор босса использован! Попробуйте уровень %d снова" % current_level,
			"zone_changed": false,
			"zone_name": "",
		}
	...
	return {
		...
		"not_enough_gold": false,
		"boss_failed": true,
		"status_text": "Босс не пройден! Возврат на уровень %d" % current_level,
		"zone_changed": false,
		"zone_name": "",
	}
...
func travel_to_level(level: int) -> Dictionary:
	if not can_travel_to_level(level):
		return _make_purchase_result("Уровень %d заблокирован" % level)
	...
	return {
		...
		"upgraded": false,
		"not_enough_gold": false,
		"status_text": "Переход на уровень %d" % current_level,
		"zone_changed": false,
		"zone_name": "",
		"travelled": true,
	}
...
func get_current_background_zone_index() -> int:
	return get_current_zone_index()


func set_debug_visual_test_mode_enabled(enabled: bool) -> void:
	debug_visual_test_mode_enabled = enabled and _debug_visual_test_mode_allowed()
	setup_current_level()
...
func is_debug_visual_test_mode_enabled() -> bool:
	return debug_visual_test_mode_enabled and _debug_visual_test_mode_allowed()


# Y4.3: debug visual test mode must never run in a Yandex release build, even
# if a stray toggle survives from elsewhere. Editor/debug builds are unaffected.
func _debug_visual_test_mode_allowed() -> bool:
	if Platform.get_platform_key() == "yandex" and not BuildConfig.IS_DEBUG_BUILD:
		return false
	return true
...
func debug_damage_current_target_by_percent(percent: float) -> Dictionary:
	if not debug_visual_test_mode_enabled:
		return _make_attack_result(false, false, BigNumber.zero(), BigNumber.zero(), target_hp.clone(), target_hp.clone(), "Режим визуальной отладки выключен")
	if target_hp.is_zero():
		return _make_attack_result(false, false, BigNumber.zero(), BigNumber.zero(), target_hp.clone(), target_hp.clone(), "Цель уже побеждена")
	...
	return _make_attack_result(
		...
		target_hp_before,
		target_hp.clone(),
		"Отладочный урон: -%s HP" % NumberFmt.compact(damage_dealt)
	)
...
func debug_clear_current_level_for_visual_test() -> Dictionary:
	if not debug_visual_test_mode_enabled:
		return {
			"cleared": false,
			"advanced_to_next_level": false,
			"status_text": "Режим визуальной отладки выключен"
		}
	...
	return {
		...
		"zone_name": zone_name,
		"status_text": "Отладка: переход на уровень %d" % current_level
	}
