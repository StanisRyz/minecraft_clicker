...
static func get_building_short_effect_description(state: ClickerState, building_index: int) -> String:
	...
	if building_index >= SettlementConfig.BUILDING_BONUS_TYPES.size():
		return "+%.1f%% Бонус" % amount

	match SettlementConfig.BUILDING_BONUS_TYPES[building_index]:
		"partner_dps":
			return "+%.1f%% DPS" % amount
		"gold":
			return "+%.1f%% Золото" % amount
		"click_damage":
			return "+%.1f%% Урон клика" % amount
		"ability_duration":
			return "+%.1f%% Длительность Фокуса/Сбора" % amount
		"ability_cooldown":
			return "+%.1f%% Перезарядка" % amount
		"boss_gold":
			return "+%.1f%% Золото босса" % amount
		_:
			return "+%.1f%% Бонус" % amount
...
static func get_prestige_talent_description(state: ClickerState, talent_index: int) -> String:
	...
	match PrestigeConfig.TALENT_BONUS_TYPES[talent_index]:
		"click_damage":
			return "+%d%% Урон клика за уровень" % amount
		"gold":
			return "+%d%% Золото за уровень" % amount
		"partner_dps":
			return "+%d%% DPS партнёров за уровень" % amount
		"autoclick_rate":
			return "+%d%% Скорость автоклика за уровень" % amount
		"settlement_effect":
			return "+%d%% Бонус поселения за уровень" % amount
		"boss_damage":
			return "+%d%% Урон боссу за уровень" % amount
		_:
			return "+%d%% Бонус за уровень" % amount
...
static func get_ability_description(state: ClickerState, ability_id: String) -> String:
	...
	match ability_id:
		"autoclick":
			...
			if not purchased:
				return _fmt("ability.description.autoclick.unlock", {"hits": int(base_hits), "duration": base_dur})
			if rank >= state.ability_max_rank:
				return _fmt("ability.description.autoclick.max", {"hits": hits, "duration": dur})
			return _fmt("ability.description.autoclick.next", {"hits": hits, "duration": dur, "rate": rate_pct, "bonus_duration": dur_bonus})
		"gold_bonus":
			...
			if not purchased:
				return _fmt("ability.description.gold_bonus.unlock", {"multiplier": "%.2f" % BalanceConfig.ABILITY_BASE_MULTIPLIER})
			if rank >= state.ability_max_rank:
				return _fmt("ability.description.gold_bonus.max", {"multiplier": "%.2f" % mult})
			return _fmt("ability.description.gold_bonus.next", {"multiplier": "%.2f" % mult, "next": "%.2f" % next_mult})
		"focus_burst":
			var mult: float = BalanceConfig.ABILITY_BASE_MULTIPLIER + BalanceConfig.ABILITY_RANK_MULTIPLIER_STEP * rank
			var next_mult: float = mult + BalanceConfig.ABILITY_RANK_MULTIPLIER_STEP
			if not purchased:
				return _fmt("ability.description.focus_burst.unlock", {"multiplier": "%.2f" % BalanceConfig.ABILITY_BASE_MULTIPLIER})
			if rank >= state.ability_max_rank:
				return _fmt("ability.description.focus_burst.max", {"multiplier": "%.2f" % mult})
			return _fmt("ability.description.focus_burst.next", {"multiplier": "%.2f" % mult, "next": "%.2f" % next_mult})
		"rally":
			var mult: float = BalanceConfig.ABILITY_BASE_MULTIPLIER + BalanceConfig.ABILITY_RANK_MULTIPLIER_STEP * rank
			var next_mult: float = mult + BalanceConfig.ABILITY_RANK_MULTIPLIER_STEP
			if not purchased:
				return _fmt("ability.description.rally.unlock", {"multiplier": "%.2f" % BalanceConfig.ABILITY_BASE_MULTIPLIER})
			if rank >= state.ability_max_rank:
				return _fmt("ability.description.rally.max", {"multiplier": "%.2f" % mult})
			return _fmt("ability.description.rally.next", {"multiplier": "%.2f" % mult, "next": "%.2f" % next_mult})
		_:
			return ""
...
static func get_skill_name(skill: Dictionary) -> String:
	var rank: String = _skill_rank(int(skill.get("skill_level", 1)))
	if skill.has("partner_index"):
		if String(skill.get("bonus_type", "")) == "click_damage_from_partner_dps":
			return _fmt("skill.partner.click_synergy.name")
		return _fmt("skill.partner.dps_mastery.name", {"rank": rank})
	var ability_id: String = String(skill.get("ability_id", ""))
	if ability_id != "":
		return _fmt("skill.ability.name", {
			"ability": _fmt("ability.%s.name" % ability_id),
			"rank": rank,
		})
	match String(skill.get("bonus_type", "")):
		"click_damage": return _fmt("skill.hero.click_damage.name", {"rank": rank})
		"partner_dps": return _fmt("skill.hero.partner_dps.name", {"rank": rank})
		"gold": return _fmt("skill.hero.gold.name", {"rank": rank})
	return _fmt("skill_popup.hero.title")


static func get_skill_description(skill: Dictionary) -> String:
	var bonus_type: String = String(skill.get("bonus_type", ""))
	if skill.has("partner_index"):
		if bonus_type == "click_damage_from_partner_dps":
			return _fmt("skill.partner.click_synergy.effect")
		return _fmt("skill.partner.dps_mastery.effect")
	var ability_id: String = String(skill.get("ability_id", ""))
	if ability_id != "":
		if ability_id == "autoclick":
			return _fmt("skill.ability.autoclick.effect", {
				"rate": str(roundi(BalanceConfig.AUTOCLICK_RANK_RATE_STEP * 100.0)),
				"duration": str(BalanceConfig.AUTOCLICK_RANK_DURATION_BONUS_SEC),
			})
		var multiplier: float = BalanceConfig.ABILITY_BASE_MULTIPLIER + BalanceConfig.ABILITY_RANK_MULTIPLIER_STEP * int(skill.get("skill_level", 1))
		return _fmt("skill.ability.%s.effect" % ability_id, {"multiplier": "%.2f" % multiplier})
	var bonus_percent: String = str(roundi(float(skill.get("bonus_value", 0.0)) * 100.0))
	match bonus_type:
		"click_damage": return _fmt("skill.hero.click_damage.effect", {"bonus": bonus_percent})
		"partner_dps": return _fmt("skill.hero.partner_dps.effect", {"bonus": bonus_percent})
		"gold": return _fmt("skill.hero.gold.effect", {"bonus": bonus_percent})
	return ""


static func _skill_rank(level: int) -> String:
	const RANKS: Array[String] = ["I", "II", "III", "IV", "V"]
	return RANKS[clampi(level, 1, RANKS.size()) - 1]
