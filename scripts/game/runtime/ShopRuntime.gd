...
static func buy_shop_products(state: ClickerState, product_id: String, mode: String) -> Dictionary:
	...
	if product.is_empty():
		return state._make_purchase_result("Неверный товар магазина")
	...
	if product_type == "permanent_multiplier":
		...
		if total_cost <= 0 or state.gems < total_cost:
			return state._make_purchase_result("Недостаточно гемов")
		...
		var perm_result: Dictionary = state._make_purchase_result(
			"%s x%d! Общий множитель: x%d" % [product_name, count, total_multiplier],
			false, true
		)
		perm_result["status_text"] = "%s куплено x%d! Всего: x%d" % [product_name, count, total_multiplier]
		return perm_result
	...
	var result: Dictionary = state._make_purchase_result("%s куплено!" % product_name, false, true)
	...
	match reward_type:
		"gold":
			...
			result["status_text"] = "%s x%d куплено! +%s золота" % [product_name, count, NumberFormatter.compact(shop_gold)]
		_:
			result["status_text"] = "Неизвестная награда магазина"
