...
func _section_zone_cycle_scaling() -> bool:
	...
	var cycle_length: int = _ZC.TOTAL_ZONE_LEVELS
	_ln("Verifies HP/reward continue growing across visual zone cycle boundaries (every %d levels)." % cycle_length)
	_ln("Visual zone/assets still cycle; only difficulty/reward multipliers scale with cycle index.")

	var check_levels: Array[int] = [
		cycle_length - 5, cycle_length - 4, cycle_length - 1, cycle_length,
		cycle_length + 1, cycle_length + 2, cycle_length + 5,
		cycle_length * 2 - 1, cycle_length * 2,
		cycle_length * 2 + 1, cycle_length * 2 + 2,
	]
	...
	# Do not compare the first normal level of a cycle to the boss at the prior cycle boundary.
	_ln("")
	_ln("  Monotonic check (normal-to-normal across cycle boundary):")
	var ok: bool = true
	var boundary_pairs: Array[Array] = [
		[cycle_length - 1, cycle_length + 1],
		[cycle_length * 2 - 1, cycle_length * 2 + 1],
	]
