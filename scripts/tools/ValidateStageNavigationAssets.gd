...
const BACKGROUND_ROOT: String = "res://assets/images/backgrounds/"
...
func _run() -> void:
	...
	_check_background_assets(expected_zone_numbers)
...
func _get_expected_zone_numbers() -> Array:
	return range(1, ZoneConfig.ZONE_COUNT + 1)


func _check_cyclic_zones() -> void:
	print("\n-- Zone cycling --")
	if ZoneConfig.ZONE_DATA.size() != ZoneConfig.ZONE_COUNT:
		_error("ZONE_DATA has %d entries; expected %d" % [ZoneConfig.ZONE_DATA.size(), ZoneConfig.ZONE_COUNT])
	if ZoneConfig.TOTAL_ZONE_LEVELS != ZoneConfig.LEVELS_PER_ZONE * ZoneConfig.ZONE_COUNT:
		_error("TOTAL_ZONE_LEVELS is not derived from LEVELS_PER_ZONE * ZONE_COUNT")
	var cases: Array = [
		[1, 1], [5, 1], [6, 2], [50, 10],
		[51, 1], [55, 1], [56, 2], [100, 10], [101, 1],
	]
	for c in cases:
		var level: int = c[0]
...
func _check_background_assets(expected_zone_numbers: Array) -> void:
	print("\n-- Zone backgrounds --")
	for n in expected_zone_numbers:
		var path: String = BACKGROUND_ROOT + "zone_%02d/background.png" % n
		if ResourceLoader.exists(path):
			print("  FOUND   %s" % path)
		else:
			_error("missing required background: %s" % path)
...
func _check_path_logic() -> void:
	...
	var cases: Array = [
		[1,   "zone_01"], [6,   "zone_02"], [26,  "zone_06"],
		[31,  "zone_07"], [41,  "zone_09"], [50,  "zone_10"],
		[51,  "zone_01"], [56,  "zone_02"], [100, "zone_10"],
		[101, "zone_01"],
	]
	for c in cases:
		var level: int = c[0]
